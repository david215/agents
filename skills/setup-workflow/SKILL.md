...
- `git log --no-merges --format=%s -30` — three answers at once: the language subjects are written in, whether Conventional Commit prefixes are in use, and whether those prefixes carry scopes (`feat(submission):` versus `feat:`). If scoped, the set of scopes actually used is the vocabulary to record. **`--no-merges` is load-bearing:** a host's merge commits (`Merged PR 5602: …`) are generated text that follows no convention, and on a squash-free repo they are a third of the sample. An empty or one-commit history settles nothing; default to English and say so.
